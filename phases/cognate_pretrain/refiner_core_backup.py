...
# Import local components (they exist in the same directory)
...
# Import local LTM Bank (exists in same directory)
from ltm_bank import create_ltm_controllers, create_memory_bank

logger.info("Successfully imported local ltm_bank")
...
class CognateRefiner(nn.Module):
    ...
    def __init__(self, config: CognateConfig):
        ...
        self.read_controller, self.write_controller, _ = create_ltm_controllers(
            d_model=config.d_model,
            memory_capacity=config.mem_capacity
        )
    ...
    def forward(
        ...
        if hasattr(self.memory_bank, 'memory_keys') and self.memory_bank.memory_keys.device != device:
            self.memory_bank.memory_keys = self.memory_bank.memory_keys.to(device)
            self.memory_bank.memory_values = self.memory_bank.memory_values.to(device)
            self.memory_bank.memory_usage = self.memory_bank.memory_usage.to(device)
        ...
        if self.memory_scheduler.should_read():
            ...
            # Use the memory bank's read method directly
            memory_keys, memory_vals = self.memory_bank.read(h_pooled, k=self.config.mem_topk)
            retrieved_indices = list(range(self.config.mem_topk))
        ...
        if recent_loss is not None and self.training:
            ...
            for i in range(B):
                if self.memory_scheduler.should_write():
                    # Use memory bank's write method directly
                    wrote = self.memory_bank.write(h_cls[i], h_cls[i])
                    memory_write_stats = {"wrote": wrote, "gate_prob": 1.0 if wrote else 0.0}
                    break  # Only write once per batch

        # Memory information
        bank_stats = {
            "memory_size": getattr(self.memory_bank, 'memory_size', 0),
            "capacity": getattr(self.memory_bank, 'capacity', 0),
        }
        scheduler_stats = getattr(self.memory_scheduler, 'get_stats', lambda: {})()
    ...
    def save_pretrained(self, save_directory: str):
        ...
        # Save memory bank state (if available)
        memory_path = save_path / "memory_bank.json"
        if hasattr(self.memory_bank, 'save'):
            self.memory_bank.save(str(memory_path))
        else:
            # For SimpleLTMBank, save basic state
            memory_state = {
                "memory_size": getattr(self.memory_bank, 'memory_size', 0),
                "capacity": getattr(self.memory_bank, 'capacity', 0),
            }
    ...
    def from_pretrained(cls, load_directory: str, **kwargs):
        ...
        if memory_path.exists():
            if hasattr(model.memory_bank, 'load'):
                model.memory_bank.load(str(memory_path))
            # For SimpleLTMBank, memory state is handled via pytorch state_dict
